...
var _elementsModal_STRIPE_PUBLIC_KEY = "pk_test_12345";

var _elementsModal_stripe = Stripe(_elementsModal_STRIPE_PUBLIC_KEY);

// Change the host url to match the Ngrok URL
var _elementsModal_HOST_URL = "https://YOUR NGROK URL.ngrok.io";
...
function createPaymentIntent(content, callback) {
...
function init(content) {
  createPaymentIntent(content).then(function(result) {
    ...
    createPaymentRequest(content, result.paymentIntent);
  });
...
function stripePaymentHandler() {
  window.location.replace("http://localhost:5000/payment_intent_succeeded");
}
